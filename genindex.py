...
<header>
    ...
    for entry in sorted_entries:
        ...
        # If this is a directory, check if it contains an index.html (output file)
        subdir_index = None
        if entry.is_dir() and not entry.is_symlink():
            icon_xlink = 'folder'
            css_class_svg = "folder_filled"
            # On non-Windows, append trailing slash
            if os.name not in ('nt',):
                entry_path = os.path.join(entry.name, '')
            # Check if subdir contains an index.html
            sub_index_path = Path(entry, opts.output_file)
            if sub_index_path.exists():
                subdir_index = f"{quote(entry_path)}/{opts.output_file}"
        ...
        # If this is a directory with an index.html, link to it
        if subdir_index:
            link_href = subdir_index
        else:
            link_href = quote(entry_path)
